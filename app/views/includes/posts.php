<!-- Post form section -->
     <div class="post-box">
        ...
        <form action="/educhat/<?php echo $_SESSION['user']['role']?>/create-post" id="postForm" method="post"  enctype="multipart/form-data" style="display:none">
            ...
            <?php foreach ($posts->index() as $key => $post):?>
                <div class="post">
                    ...
                    <?php if ($post['image_path']!==null):?>
                        <img src="/educhat/public<?php echo $post['image_path'];?>" alt="photo">
                    <?php endif;?>
                    <span class="timestamp">Last updated: <?php echo $post['timeDiff']." ".$post['unit']." ago.";?></span>
                    <a href="<?php echo "/educhat/{$_SESSION['user']['role']}/posts/".$post['id'];?>">View Comments</a>
                    <?php if($post['user_id']===$_SESSION['user']['id'] || $_SESSION['user']['role']==='school'):?>
                    <a href="<?php echo "/educhat/{$_SESSION['user']['role']}/post/delete/".$post['id'];?>" class="delete-btn" id="confirmBtn">Delete Post</a>
                    <?php endif;?>
                </div>
            <?php endforeach;?>
        <?php else:?>
